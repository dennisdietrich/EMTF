    ...
    public class ConsoleLogger : Logger
    {
        ...
        protected override void TestRunCompleted(TestRunCompletedEventArgs e)
        {
            ...
            else
            {
                ...
                Console.Write(new String('-', Console.WindowWidth));
                Console.WriteLine(String.Format(CultureInfo.CurrentCulture, "Test run completed execution of {0:N0} tests in {1:N0} seconds.", e.Total, (e.EndTime - e.StartTime).TotalSeconds));
            ...
            lock (_syncRoot)
            {
                ...
                switch (e.Reason)
                {
                    ...
                    case SkipReason.TestActionAttributeDefined:
                        Console.WriteLine(String.Format(CultureInfo.CurrentCulture,
                                                        "PreTestAction or PostTestAction attribute defined on {0}",
                                                        UseFullTestName ? e.FullTestName : e.TestName));
                        break;
                    default:
