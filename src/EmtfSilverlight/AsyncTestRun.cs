...
namespace Emtf
{
    internal sealed class AsyncTestRun : IAsyncResult, IDisposable
    {
...
#if !SILVERLIGHT
        internal AsyncTestRun(TestExecutor executor, AsyncCallback callback, Object state)
            : this(new BaseParameters(executor, callback, state))
        {
        ...
        internal AsyncTestRun(TestExecutor executor, IEnumerable<Assembly> assemblies, AsyncCallback callback, Object state)
            : this(new BaseParameters(executor, callback, state))
        {
        ...
        internal AsyncTestRun(TestExecutor executor, IEnumerable<MethodInfo> methods, AsyncCallback callback, Object state)
            : this(new BaseParameters(executor, callback, state))
        {
            _methods       = methods;
            ...
            try
            {
                switch (_parameterType)
                {
                    ...
                    case ParameterType.IEnumerableOfMethodInfo:
                        _executor.Execute(_methods);
                        break;
                    case ParameterType.IEnumerableOfAssembly:
                        _executor.Execute(_assemblies);
                        break;
                    default:
        ...
        {
            internal BaseParameters(TestExecutor executor, AsyncCallback callback, Object state)
            {
                Executor = executor;
                Callback = callback;
                State    = state;
